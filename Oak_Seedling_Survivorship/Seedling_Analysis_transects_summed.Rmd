...
Testing nbinom1
```{r}
alive_mod_nb1 <- update(alive_mod1, family = nbinom1())
model.sel(list(alive_mod1, alive_mod_nb1))
#worse fit
```
...
* Also need to keep in mind that Plot is not a significant variable in the proportion dead model
...
```{r}
prop_mod_herb <- update(prop_mod, ~ Site + herb_YN + (1|RE))

model.sel(list(prop_mod, prop_mod_herb))
...
june18_dat %>% 
  ...
  bar_plot +
  error_bars
#pretty interesting, addition of cattle clearly increases above-ground herbivory
...
```{r}
prop_mod_above <- update(prop_mod, ~ Site + above_prop + (1|RE))
#did this work? not sure why cnd(abv_prp) is a numeric value and not just a "+" like it usually is
model.sel(list(prop_mod, prop_mod_above))
```

### Modelling Above-Ground Herbivory

```{r}
above_mod1 <- glmmTMB(above_prop ~ Plot + Site + (1|RE), ziformula = ~ 1, family = beta_family(link = "probit"), data = june18_dat)
#doesn't converge with interaction
...
dredge(above_mod1, varying = zi_list, subset = ("cond(Site)" & "cond(Plot)"))
#zi = ~ 1 is best
...
```{r}
above_mod <- glmmTMB(above_prop ~ Plot + (1|RE), ziformula = ~ 1, family = beta_family(link="cloglog"), data = june18_dat)
summary(above_mod)
```

DHARMa Diagnostics:
```{r}
above_sim <- simulateResiduals(fittedModel = above_mod, plot = F, n = 1000)
hist(above_sim)
testResiduals(above_sim, plot = T)
#uniformity, dispersion, and outlier tests all non-significant
#dispersion isn't great, cloglog looks to be better than logit or probit
```

Testing Effects
```{r}
plot(allEffects(above_mod))
#not sure why this doesn't work, might be becasue of ZI?
