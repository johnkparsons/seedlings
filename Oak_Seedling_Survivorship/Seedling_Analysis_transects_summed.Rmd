...
above_plot <- june18_dat %>% 
  ...
  ggplot(aes(x = Site, y = y, fill = Plot)) +
    ...
    error_bars +
    labs(x = "Climate Site", 
         y = "Proportion of Seedlings", 
         title = "Above-Ground Herbivory Occurrence by Climate Site and Grazing Treatment", 
         fill = "Grazing Treatment") +
    scale_fill_brewer(palette = "Dark2")
above_plot
...
summary_table_se <- june18_dat %>% 
  group_by(Plot, Site) %>% 
  summarize_at(vars(total, alive, prop_dead, above_prop), funs(std.error(.))) %>% 
  ungroup() %>% 
  t() %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
