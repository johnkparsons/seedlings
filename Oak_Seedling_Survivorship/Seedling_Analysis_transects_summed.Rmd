...
#site contrasts are all non-significant
emmeans(total_mod, pairwise ~ Plot)
#plot contrasts are all significant
...
* report this?
* probably more I can do to investigate interaction
...
#start with binomial distribution (two approaches, cbind vs weight by total)
prop_mod0 <- glmmTMB((alive/total) ~ Site * Plot + (1|RE), ziformula = ~ 0, data = june18_dat, family = binomial(), weights = (total))
prop_mod1 <- glmmTMB(cbind(alive, dead) ~ Site * Plot + (1|RE), ziformula = ~ 0, data = june18_dat, family = binomial())

prop_mod2 <- update(prop_mod1, ~ Site * Plot)

model.sel(list(prop_mod1, prop_mod0))
#RE does improve fit
```
...
*dredge doesn't work with either model

```{r}
prop_mod01 <- update(prop_mod0, ~ Site + Plot + (1|RE))
prop_mod02 <- update(prop_mod0, ~ Plot + (1|RE))
prop_mod03 <- update(prop_mod0, ~ Site + (1|RE))
prop_mod04 <- update(prop_mod0, ~ (1|RE))
                     
model.sel(list(prop_mod0, prop_mod01, prop_mod02, prop_mod03, prop_mod04))
#full model is best

prop_mod2 <- update(prop_mod1, ~ Site + Plot + (1|RE))
prop_mod3 <- update(prop_mod1, ~ Plot + (1|RE))
prop_mod4 <- update(prop_mod1, ~ Site + (1|RE))
prop_mod5 <- update(prop_mod1, ~(1|RE))

model.sel(list(prop_mod1, prop_mod2, prop_mod3, prop_mod4))
#full model is best
```
...
* zi = (1|Site) is best, but I'll stick with ~1 for now (code isn't knitting either)

DHARMa Diagnostics:
```{r}
prop_dead_sim0 <- simulateResiduals(fittedModel = prop_mod0, plot = F, n = 1000)
hist(prop_dead_sim0)
testResiduals(prop_dead_sim0, plot = T)
#uniformity, dispersion, and outlier tests all non-significant

prop_dead_sim1 <- simulateResiduals(fittedModel = prop_mod1, plot = F, n = 1000)
hist(prop_dead_sim1)
testResiduals(prop_dead_sim1, plot = T)
#uniformity, dispersion, and outlier tests all non-significant
```

* Model 1, with cbind(alive, dead), looks a bit better on the QQ plot
...
pairs(emmeans(prop_mod1, "Plot", "Site"))
# Arid O-W and O-WC are only significant pairs

pairs(emmeans(prop_mod1, "Site"))
# Arid-Interm are significantly different when plots are averaged, idk how arid-mesic isn't significant as well?
```
...
herb_mod <- update(herb_mod1, ~ (1|RE))
summary(herb_mod)
```
...
```{r echo=FALSE}
prop_mod_herb <- update(prop_mod0, ~ Site + herb_YN + (1|RE))
...
```{r}
prop_mod_above <- update(prop_mod0, ~ Site + above_prop + (1|RE))
