...
#test random effects:
total_mod2 <- update(total_mod1, ~ Plot*Site) #no Site/Block
...
#test random effects:
alive_mod2 <- update(alive_mod1, ~ Plot*Site) #no Site/Block
alive_mod3 <- update(alive_mod1, ~ Plot*Site + (1 + Plot|Site/Block)) #random slope of Plot
alive_mod4 <- update(alive_mod1, ~ Plot*Site + (1 + Site|Site/Block)) #random slope of Site
alive_mod5 <- update(alive_mod1, ~ Plot*Site + (0 + Plot|Site/Block)) #random slope of Plot, no variation in intercept
alive_mod6 <- update(alive_mod1, ~ Plot*Site + (0 + Site|Site/Block)) #random slope of Site, no variation in intercept
...
alive_mod <- glmmTMB(alive ~ Plot + (1|Site/Block), ziformula = ~ 1, data = june18_dat, family = nbinom2())
summary(alive_mod)
```
...
prop_mod0 <- glmmTMB((alive/total) ~ Site * Plot + (1|Site/Block), ziformula = ~ 0, data = june18_dat, family = binomial(), weights = (total))

prop_mod1 <- glmmTMB(cbind(alive, dead) ~ Site * Plot + (1|Site/Block), ziformula = ~ 0, data = june18_dat, family = binomial())

prop_mod2 <- update(prop_mod1, ~ Site * Plot)

model.sel(list(prop_mod1, prop_mod0))
#Site/Block does improve fit
```
...
```{r}
prop_mod01 <- update(prop_mod0, ~ Site + Plot + (1|Site/Block))
prop_mod02 <- update(prop_mod0, ~ Plot + (1|Site/Block))
prop_mod03 <- update(prop_mod0, ~ Site + (1|Site/Block))
prop_mod04 <- update(prop_mod0, ~ (1|Site/Block))
                     
model.sel(list(prop_mod0, prop_mod01, prop_mod02, prop_mod03, prop_mod04))
#full model is best

prop_mod2 <- update(prop_mod1, ~ Site + Plot + (1|Site/Block))
prop_mod3 <- update(prop_mod1, ~ Plot + (1|Site/Block))
prop_mod4 <- update(prop_mod1, ~ Site + (1|Site/Block))
prop_mod5 <- update(prop_mod1, ~(1|Site/Block))
...
```{r echo=TRUE}
above_mod1 <- glmmTMB(above_prop ~ Plot + Site + (1|Site/Block), ziformula = ~ 1, family = beta_family(link = "probit"), data = june18_dat)
...
```{r echo=TRUE}
above_mod <- glmmTMB(above_prop ~ Plot + (1|Site/Block), ziformula = ~ 1, family = beta_family(link="cloglog"), data = june18_dat)
summary(above_mod)
```
